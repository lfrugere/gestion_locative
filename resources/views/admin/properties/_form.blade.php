...
<form class="card" method="POST" action="{{ $action }}">
    ...
    <div class="form-grid">
        ...
        <div class="form-field"><label for="bank_account_id">Compte bancaire</label><select id="bank_account_id" name="bank_account_id"><option value="">Aucun</option>@foreach ($bankAccounts as $bankAccount)<option value="{{ $bankAccount->id }}" @selected((string) old('bank_account_id', $property->bank_account_id ?? '') === (string) $bankAccount->id)>{{ $bankAccount->label }}</option>@endforeach</select></div>
