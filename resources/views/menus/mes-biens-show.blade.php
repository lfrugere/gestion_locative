...
@section('content')
    ...
    <section class="detail-hero">
        <div class="detail-hero-copy">
            <a class="back-link" href="{{ route('mes-biens') }}">← Mes biens</a>
        ...
        <div class="detail-main">
            ...
            <section class="detail-panel address-panel">
                ...
                @if ($property->building)
                    <a class="related-entity" href="{{ route('mes-immeubles.show', $property->building) }}"><span class="entity-mark">I</span><span><strong>{{ $property->building->name }}</strong><small>{{ $mapAddress->line1 }}, {{ $mapAddress->city }}</small></span><span class="row-arrow">→</span></a>
            ...
            @if ($property->bankAccount)
                <section class="detail-panel">
                    ...
                    <a class="related-entity" href="{{ route('mes-comptes-bancaires.show', $property->bankAccount) }}"><span class="entity-mark">B</span><span><strong>{{ $property->bankAccount->label }}</strong></span><span class="row-arrow">→</span></a>
                </section>
            @endif

            @if ($isPropertyManager && ! auth()->user()->hasRole('admin'))
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @if ($invoice->bankTransaction)
                                                    <a href="{{ route('mes-comptes-bancaires.show', $property->bank_account_id) }}">Ajoutée au compte</a>
                                                @else
            ...
            @if ($property->canHaveRooms())
                <section class="detail-panel">
                    <div class="panel-heading"><div><span class="panel-kicker">Colocation</span><h2>Pièces</h2></div></div>
