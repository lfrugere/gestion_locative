...
- L’entrypoint Docker exécute `php artisan migrate --force`, puis `php artisan db:seed --force`, avant Apache. Le seeder est idempotent : les rôles et permissions sont retrouvés ou créés, et le compte administrateur existant par e-mail n’est pas recréé ni réinitialisé.
