...
| Adresse | Contient latitude, longitude et date de géocodage quand le géocodage aboutit. Le pays est toujours France (FR) : l’application est un usage strictement personnel et local, le champ n’est pas saisi et n’est plus affiché que sous sa forme lisible. |
...
L'intention produit derrière ces trois rôles (qui ils sont, leur mission, le périmètre de leur responsabilité) est décrite dans [personas.md](personas.md). Le détail exhaustif par entité (qui peut créer, modifier, supprimer, et les restrictions de propriété sur les biens) est tenu à jour dans [roles-permissions.md](roles-permissions.md). Toute évolution des droits doit être répercutée dans ces deux documents, dans le seeder et dans un test fonctionnel.
